...
def paired_paths_from_folder(folders, keys, filename_tmpl):
    ...
    assert len(folders) == 2, (
        ...
        f'But got {len(folders)}')      # 断言为配对数据
    assert len(keys) == 2, (
        'The len of keys should be 2 with [input_key, gt_key]. '
        f'But got {len(keys)}')
    input_folder, gt_folder = folders   # 解包
    input_key, gt_key = keys    # 解包

    input_paths = sorted(list(scandir(input_folder)))       # 易出bug位置
    gt_paths = sorted(list(scandir(gt_folder)))             # 易出bug位置
